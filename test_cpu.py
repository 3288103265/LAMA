...
def main(args):
    ...
    dataset = get_dataset(args.dataset, img_size,
                          left_right_flip=False, train=args.train)
    if args.image_id_savepath is not None and 'coco' in args.dataset.lower():
        np.savetxt(args.image_id_savepath, np.array(
            dataset.image_ids).astype(int))
        print(
            f"Image ids of generated images in COCO are save in {args.image_id_savepath}.")
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=1,
        drop_last=False, shuffle=False, num_workers=1)

    netG = globals()[f'ResnetGenerator{img_size}'](
        num_classes=num_classes, output_dim=3)
    ...
    repeat = args.repeat
    save_path = os.path.join(args.sample_path, args.dataset) + \
        f"{img_size}_repeat{repeat}_thres{thres:.1f}"
    ...
    for idx, data in tqdm(enumerate(dataloader)):
        ...
        real_image, label = real_image, label.long().unsqueeze(-1)
        fake_images = []
        for r in range(repeat):
            z_obj = truncted_random(num_o=bbox.size(1), thres=thres)
            z_im = truncted_random(num_o=1, thres=thres).view(1, -1)
            with torch.no_grad():
                fake_image = netG.forward(
                    z_obj, bbox.float(), z_im, label.squeeze(dim=-1))
                fake_images.append(fake_image)
            fake_arrays = fake_image[0].cpu().detach().mul(0.5).add(
                0.5).mul(255).to(torch.uint8).numpy().transpose(1, 2, 0)
            if not args.NOSaving:
                imageio.imwrite(
                    f"{save_path}/sample_{idx}_{r}.png", fake_arrays)
            ...
            if args.cropped_size > 0:
                ...
                bbox_rec[:, :, 3] = bbox[:, :, 3] + \
                    bbox[:, :, 1]  # y2 = y1 + h
                bbox_rec = bbox_rec.mul(real_image.size(2)).to(
                    torch.int)  # the position
                for idx1, bl in enumerate(zip(bbox_rec[0], label.flatten().tolist())):
                    b, l = bl[0].tolist(), bl[1]
                    if l > 0:  # remove background
                        cropped = fakeImage.crop(b).resize(
                            (args.cropped_size, args.cropped_size), resample=Image.BICUBIC)
                        cropped.save(os.path.join(
                            save_path, str(l), f"{idx}_{r}_{idx1}.png"))
