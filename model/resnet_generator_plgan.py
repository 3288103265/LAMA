...
class ResnetGenerator128(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3, instance_threshold=92):
        ...
        self.res1 = ResBlock(ch*16, ch*16, upsample=True, num_w=num_w, predict_mask=True, psp_module=False,
                             Norm=SpatialAdaptiveSynBatchNorm2d)
        self.res2 = ResBlock(ch*16, ch*8, upsample=True, num_w=num_w, predict_mask=True, psp_module=False,
                             Norm=SpatialAdaptiveSynBatchNorm2d)
        self.res3 = ResBlock(ch*8, ch*4, upsample=True, num_w=num_w, predict_mask=True, psp_module=False,
                             Norm=SpatialAdaptiveSynBatchNorm2d)
        self.res4 = ResBlock(ch*4, ch*2, upsample=True, num_w=num_w, predict_mask=True, psp_module=True,
                             Norm=SpatialAdaptiveSynBatchNorm2d)
        self.res5 = ResBlock(ch*2, ch*1, upsample=True, num_w=num_w, predict_mask=False, psp_module=False,
                             Norm=SpatialAdaptiveSynBatchNorm2d)
        ...
        self.m_cgf2 = ISA(num_classes, in_ch=ch * 16, norm=SynchronizedBatchNorm2d)
        self.m_cgf3 = ISA(num_classes, in_ch=ch * 8, norm=SynchronizedBatchNorm2d)
        self.m_cgf4 = ISA(num_classes, in_ch=ch * 4, norm=SynchronizedBatchNorm2d)
        self.m_cgf5 = ISA(num_classes, in_ch=ch * 2, norm=SynchronizedBatchNorm2d)
    ...
    def forward(self, z, bbox=None,  z_im=None, y=None, vis=False, bbox_lt=None, attr=None,):
        ...
        if self.training:
            ...
            w = self.context(w, w, w, bbox, y).view(b * o, -1)
            ...
            if bbox is not None:
                ...
                w = self.context(w, w, w, torch.cat([bbox_lt, bbox_wh], 2), y).view(b * o, -1)
        ...
        x, stage_mask = self.res1(x, w, panoptic_bbox1)
        # 16x16
        stage_bbox, panoptic_bbox2 = self.m_cgf2(x, stage_mask, bbox, bbox_mask_, y, b, o, seg, y_instance_mask)
        x, stage_mask = self.res2(x, w, panoptic_bbox2)
        # 32x32
        stage_bbox, panoptic_bbox3 = self.m_cgf3(x, stage_mask, bbox, bbox_mask_, y, b, o, seg, y_instance_mask)
        x, stage_mask = self.res3(x, w, panoptic_bbox3)
        # 64x64
        stage_bbox, panoptic_bbox4 = self.m_cgf4(x, stage_mask, stage_bbox, bbox_mask_, y, b, o, seg, y_instance_mask)
        x, stage_mask = self.res4(x, w, panoptic_bbox4)
        # 128x128
        stage_bbox, panoptic_bbox5 = self.m_cgf5(x, stage_mask, stage_bbox, bbox_mask_, y, b, o, seg, y_instance_mask)
        ...
        if vis:
            ...
            res_vis = {
                ...
                # 'weight feat': weight[:vis_num],
                # 'bias feat': bias[:vis_num],
...
class LostGANGenerator128(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3, instance_threshold=92):
        super(LostGANGenerator128, self).__init__()
        self.num_classes = num_classes
        self.map_size = 64
        self.instance_threshold = instance_threshold
        self.mask_embedding = nn.Embedding(num_classes, 180)
    ...
    def forward(self, z, bbox=None, bbox_lt=None, attr=None, z_im=None, y=None, vis=False):
        ...
        if vis:
            ...
            return x, bbox_wh, mask, res_vis
        return x, bbox_wh, panoptic_bbox5
...
class PSPModule(nn.Module):
    ...
    def _make_stage(self, features, out_features, size):
        ...
        bn = SynchronizedBatchNorm2d(out_features)
...
class ISA(nn.Module):
    def __init__(self, num_classes, in_ch=3, radius=1, norm=nn.BatchNorm2d):
        ...
        self.alpha = nn.Parameter(torch.zeros(1, num_classes, 1))
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, stage_mask, bbox, bbox_mask_, y, b, o, seg, y_instance_mask):
        hh, ww = x.size(2), x.size(3)
        bbox = F.interpolate(bbox, size=(hh, ww), mode='bilinear')
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.view(b, o, 1, 1))
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(hh, ww), mode='nearest')
        alpha = torch.gather(self.sigmoid(self.alpha).expand(b, -1, -1), dim=1, index=y.view(b, o, 1)).unsqueeze(-1)
        bbox = F.interpolate(bbox, size=(hh, ww), mode='bilinear') * (1 - alpha) + seman_bbox * alpha
    ...
    def mask_filter(self, guide, mask):
        ...
        mask_filtered = torch.mean(A * guide + b, dim=1, keepdim=True)
